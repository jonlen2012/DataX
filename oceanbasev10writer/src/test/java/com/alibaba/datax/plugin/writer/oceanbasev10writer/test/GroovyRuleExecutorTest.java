...
public class GroovyRuleExecutorTest {
	...
	public void testExecuteGroovy() throws Exception {
		...
		Assert.assertEquals(result, "test_0015");
	...
	public void testExecuteStringGroovy() throws Exception {
		...
		GroovyRuleExecutor groovyRule = new GroovyRuleExecutor(
				"String table_index = #id#.substring(13,15); int temp = Integer.parseInt(table_index); return String.format(\"%d\",(Integer)temp % 100);",
				"test_{0}");
		String result = groovyRule.executeRule(columnValues);
		System.out.println(result + ":" + groovyRule.executeRule(columnValues));
	}

	@Test
	public void testRuleExecute() {
		Map<String, Object> columnValues = new HashMap<String, Object>();
		GroovyRuleExecutor groovyRule = new GroovyRuleExecutor("((#id#).longValue() % 8).intdiv(4)",
				"datax_3_mysqlrulewriter_{00}");
