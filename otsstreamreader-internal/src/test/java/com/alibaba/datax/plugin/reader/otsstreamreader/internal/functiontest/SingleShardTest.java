...
public class SingleShardTest {
    ...
    private static OTSAsync otsAsync = ConfigurationHelper.getOTSAsyncFromConfig();
    ...
    private void getStreamRecords() {
        GetStreamRecordResult getStreamRecordResult = ots.getStreamRecord(new GetStreamRecordRequest(StreamHelper.getShardIterator(ots, tableName)));
        System.out.println(getStreamRecordResult.getRecords().get(1));
    }
        ...
        for (int i = 0; i < opNum; i++) {
            ...
            switch (opType) {
                case 0: {
                    ...
                    otsAsync.putRow(putRowRequest, new OTSCallback<PutRowRequest, PutRowResult>() {
                        @Override
                ...
                case 1: {
                    ...
                    otsAsync.updateRow(updateTableRequest, new OTSCallback<UpdateRowRequest, UpdateRowResult>() {
                        @Override
                        public void onCompleted(OTSContext<UpdateRowRequest, UpdateRowResult> otsContext) {
                            semaphore.release();
                        }

                        @Override
                        public void onFailed(OTSContext<UpdateRowRequest, UpdateRowResult> otsContext, OTSException ex) {
                            ex.printStackTrace();
                        }

                        @Override
                        public void onFailed(OTSContext<UpdateRowRequest, UpdateRowResult> otsContext, ClientException ex) {
                            ex.printStackTrace();
                        }
                    });
                    break;
                ...
                case 2: {
                    ...
                    otsAsync.deleteRow(deleteRowRequest, new OTSCallback<DeleteRowRequest, DeleteRowResult>() {
                        @Override
                        public void onCompleted(OTSContext<DeleteRowRequest, DeleteRowResult> otsContext) {
                            semaphore.release();
                        }

                        @Override
                        public void onFailed(OTSContext<DeleteRowRequest, DeleteRowResult> otsContext, OTSException ex) {
                            ex.printStackTrace();
                        }

                        @Override
                        public void onFailed(OTSContext<DeleteRowRequest, DeleteRowResult> otsContext, ClientException ex) {
                            ex.printStackTrace();
                        }
                    });
                    break;
                ...
                case 3: {
                    ...
                    otsAsync.batchWriteRow(batchWriteRowRequest, new OTSCallback<BatchWriteRowRequest, BatchWriteRowResult>() {
                        @Override
                        public void onCompleted(OTSContext<BatchWriteRowRequest, BatchWriteRowResult> otsContext) {
                            semaphore.release();
                        }

                        @Override
                        public void onFailed(OTSContext<BatchWriteRowRequest, BatchWriteRowResult> otsContext, OTSException ex) {
                            ex.printStackTrace();
                        }

                        @Override
                        public void onFailed(OTSContext<BatchWriteRowRequest, BatchWriteRowResult> otsContext, ClientException ex) {
                            ex.printStackTrace();
                        }
                    });
                }
        ...
        switch (opType) {
            case 0: {
                for (int i = 0; i < rowNum; i++) {
                    ...
                    request.addRowPutChange(putRowRequest.getRowChange());
                }
                break;
            }
            case 1: {
                for (int i = 0; i < rowNum; i++) {
                    ...
                    request.addRowUpdateChange(updateRowRequest.getRowChange());
                }
                break;
            }
            case 2: {
                for (int i = 0; i < rowNum; i++) {
                    DeleteRowRequest deleteRowRequest = genDeleteRowRequest(tableName);
                    request.addRowDeleteChange(deleteRowRequest.getRowChange());
                }
            }
        }
        return request;
