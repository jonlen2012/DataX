...
public class WriterModelParseUnittest {
    ...
    public void testParseOTSPKColumn() {
        ...
        }
        {
            ...
            t.put("name", "xx");
            t.put("type", "hello");
            input.put(t, "Primary key type only support 'string' and 'int', not support 'hello'.");
        }
        {
        ...
        }
        {
            ...
            t.put("name", "xx");
            t.put("type", "double");
            input.put(t, "Primary key type only support 'string' and 'int', not support 'double'.");
        }
        {
            ...
            t.put("name", "xx");
            t.put("type", "bool");
            input.put(t, "Primary key type only support 'string' and 'int', not support 'bool'.");
        }
        {
            // 传入了类型为Binary的Column, {"name":"xx", "type":"binary"}
            Map<String, Object> t = new LinkedHashMap<String, Object>();
            t.put("name", "xx");
            t.put("type", "binary");
            input.put(t, "Primary key type only support 'string' and 'int', not support 'binary'.");
        }
     ...
     */
    @Test
    public void testParseOTSAttrColumn() {
        ...
        for (Entry<Map<String, Object>, String> e : input.entrySet()) {
            LOG.info("Map: {}, Expect: {}", g.toJson(e.getKey()), e.getValue());
            try {
                WriterModelParser.parseOTSAttrColumn(e.getKey());
                assertTrue(false);
            } catch (Exception ee) {
                assertEquals(e.getValue(), ee.getMessage());
            }
