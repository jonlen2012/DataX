...
public class HBaseBulker {
    ...
    public int init(String table, org.apache.hadoop.conf.Configuration hbaseConf,
            ...
            throws IOException {
        this.table = table;
        this.htable = HBaseHelper.getTable(hbaseConf, table);
    ...
    public int init(Configuration dataxConf) throws IOException {
        ...
        try {
            htable = HBaseHelper.getTable(hbaseConf, table);
        } catch (IOException e) {
    ...
    public int post() {
        ...
        try {
            ...
            if (isTruncateTable) {
                Admin hadmin = HBaseHelper.getAdmin(hbaseConf);
                if (hadmin.isTableEnabled(TableName.valueOf(table))) {
                    hadmin.disableTable(TableName.valueOf(table));
                }
                hadmin.truncateTable(TableName.valueOf(table),false);
                hadmin.close();
            }
             ...
             * >HBASE-9537</a>
             */
            loader.doBulkLoad(new Path(uri + "/" + hdfsOutputDir), (HTable) htable);
        } catch (Exception e) {
