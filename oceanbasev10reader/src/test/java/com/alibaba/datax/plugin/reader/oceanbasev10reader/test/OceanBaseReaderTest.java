...
public class OceanBaseReaderTest extends BasicReaderPluginTest {
	...
	//
	// @TestLogger(log =
	// "测试basic1.json. 配置一个jdbcUrl,一个querySql,运行时，直接执行 queryS1ql 进行数据读取.")
	// @Test
	...
	//
	// @TestLogger(log =
	// "测试case2.json. 分库分表验证[1个库，共2条全表 querySql]. jdbcUrl + querySql ")
	// @Test
	...
	//
	// @TestLogger(log =
	// "测试case3.json，分库分表[1个库，共2张表]验证. jdbcUrl + table + where")
	// @Test
	...
	//
	// @TestLogger(log =
	// "测试case4.json，配置5个分表进行验证，但是框架建议切分为10份，用户未配置splitPk，所以切分份数最终只为表个数：5")
	// @Test
	...
	//
	// @TestLogger(log =
	// "测试case5.json，配置5个分表进行验证，但是框架建议切分为8份，用户配置splitPk，所以切分。但是切分总数是（8/5向上取整）*5 = 10")
	// @Test
	...
	//
	// @TestLogger(log =
	// "测试case6_null.json，配置5个分表进行验证，但是框架建议切分为8份，用户配置splitPk，所以切分。但是切分总数是（8/5向上取整）*5 = 10")
	// @Test
	...
	//
	// @TestLogger(log =
	// "测试basic7.json. 配置两个jdbcUrl（第一个为slave jdbcUrl，第二个为master jdbcUrl）一个table,运行时，会选择第一个jdbcUrl并通过程序自动生成 queryS1ql 进行数据读取.")
	// @Test
