...
public class MysqlReader extends Reader {
    ...
    public static class Slave extends Reader.Slave {
        ...
        public void init() {
            ...
            this.username = this.readerSliceConfig.getNecessaryValue(Key.USERNAME,
                    MysqlReaderErrorCode.REQUIRED_VALUE);
            this.password = this.readerSliceConfig.getNecessaryValue(Key.PASSWORD,
                    MysqlReaderErrorCode.REQUIRED_VALUE);
            this.jdbcUrl = this.readerSliceConfig.getNecessaryValue(Key.JDBC_URL,
                    MysqlReaderErrorCode.REQUIRED_VALUE);
        ...
        private static void dealSessionConf(Connection conn,
            ...
            try {
                ...
                LOG.error("Origin cause:[%s], failed to createStatement, from:[{}].",
                        e.getMessage(), BASIC_MESSAGE);
                throw new DataXException(MysqlReaderErrorCode.SQL_EXECUTE_FAIL, e);
            }
            ...
            for (Entry<String, String> entry : sessionConfs.entrySet()) {
                ...
                LOG.info("execute sql:[{}], from:[{}].", sessionSql, BASIC_MESSAGE);
                try {
                    DBUtil.executeSqlWithoutResultSet(stmt, sessionSql);
                } catch (SQLException e) {
                    LOG.error(String.format("Origin cause:[%s], failed to execute sql:[%s], from:[%s].",
