...
public class CommonRdbmsWriter {
	...
	public static class Slave {
		...
		public void prepare(Configuration writerSliceConfig) {
			this.metaDataConnection = DBUtil.getConnection(DataBaseType.MySql,
					this.jdbcUrl, username, password);

			dealSessionConf(metaDataConnection,
			...
			if (tableNumber != 1) {
				...
				WriterUtil.executeSqls(metaDataConnection, this.preSqls,
						BASIC_MESSAGE);
			}

			// 用于写入数据的时候的类型根据目的表字段类型转换
			this.resultSetMetaData = DBUtil.getColumnMetaData(
					metaDataConnection, this.table,
		...
		public void destroy(Configuration writerSliceConfig) {
			DBUtil.closeDBResources(null, this.metaDataConnection);
		}
		...
		private PreparedStatement fillPreparedStatement(
			...
			for (int i = 0; i < this.columnNumber; i++) {
				...
				case Types.DATE:
					if (this.resultSetMetaData.getColumnTypeName(i + 1)
