...
public class CompareHelper {
     ...
     * @return
     */
    public static int comparePrimaryKeyColumnListList(List<List<PrimaryKeyColumn>> before, List<List<PrimaryKeyColumn>> after) {
        int size = before.size() < after.size() ? before.size() : after.size();
        
        for (int i = 0; i < size; i++) {
            int cmp = comparePrimaryKeyColumnList(before.get(i), after.get(i));
            if (cmp != 0) {
                return cmp;
            }
        }
