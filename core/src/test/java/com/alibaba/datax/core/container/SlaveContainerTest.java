...
public class SlaveContainerTest extends CaseInitializer {
    ...
    public void setUp() {
        ...
        this.configuration = ConfigParser.parse(path
                + File.separator + "all.json");
        ...
        this.configuration.set(CoreConstant.DATAX_CORE_CONTAINER_SLAVE_CHANNEL, channelNumber);
        Configuration jobContent = this.configuration
                .getListConfiguration(CoreConstant.DATAX_JOB_CONTENT).get(0);
    ...
    public void testReaderException() {
        this.configuration.set("plugin.reader.fakereader.class", FakeExceptionReader.class.getName());
        SlaveContainer slaveContainer = new SlaveContainer(this.configuration);
        slaveContainer.start();
    }
